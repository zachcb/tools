...
impl SyntaxNode {
	...
	pub(crate) fn replace_nodes_with_range(
		...
		// println!("Replacing: {:?}", self.text_range());
		...
		match replacer.matches(self, range) {
			...
			// TODO: Need to fix after rebasing on top of slots changes
			ReplacerMatch::Overlap(_) => todo!(),
...
impl SyntaxNodeReplacer {
	fn matches(&self, node: &SyntaxNode, search_range: Range<usize>) -> ReplacerMatch {
		...
		println!("Replacing! {:?}", node.text_range());
		...
		if has_overlap {
			...
			println!("No Overlap: {} @ {:?}", node, node.text_range());
			ReplacerMatch::None
		}
	}
}
